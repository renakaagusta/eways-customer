...
class CreateUser: ViewModel() {
    private val services = AuthApiServices::class.java
    private val result = MutableLiveData<ArrayList<User>>()

    internal fun set(username: String, fullname: String, phoneNumber: String, email: String, password: String, imagePath: String, role: Int, activeStatus: Boolean, verifStatus: Boolean) {
        MyRetrofit
            .createService(services)
            .createUser(username, fullname, phoneNumber, email, password, imagePath, role, activeStatus, verifStatus)
