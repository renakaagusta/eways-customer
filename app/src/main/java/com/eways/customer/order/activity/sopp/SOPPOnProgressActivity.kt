...
class SOPPOnProgressActivity :BaseActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CustomSupportActionBar.setCustomActionBar(this, "Detail Pesanan SOPP")
    ...
    private fun setData(){
        GlobalScope.launch(Dispatchers.Main) {
            getOrderDetail.set(id, this@SOPPOnProgressActivity)
            delay(1000)
            ...
            getAgentDetail.get().observe(this@SOPPOnProgressActivity, Observer {
    ...
    private fun setLayout(){
        if(orderStatus == OrderStatus.Created){
            ...
            llTransaction.isVisible = true
            when(paymentStatus) {
                PaymentStatus.Unpaid -> {
                    txtStatus.text = "On Progress"
                }
                PaymentStatus.Paid -> {
                    txtStatus.text = "Tagihan Sudah Dibayarkan"
                    when (paymentStatus) {
                        PaymentStatus.Paid -> {
    ...
    private fun moveToChat(){
        ...
        llChatNotReady.setOnClickListener{
            this@SOPPOnProgressActivity.showError("Agen Belum menerima pesanan")
        }
    }
                ...
                if(it.errors.message.isEmpty()){
                    GlobalScope.launch(Dispatchers.Main) {
                        createNotification.set(
                            ...
                            id,
                            "Eways",
                            "Order sopp mu telah diselesaikan user"
                        )
                        delay(300)
                        createNotification.set(
                            user.ID!!,
